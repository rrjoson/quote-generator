  ...
  render() {
    ...
    return (
      <View style={styles.container}>
        <Text style={[styles.message, isFontLoaded && { fontFamily: 'Prata' }]}>{activeQuote.message}</Text>
        <Text style={[styles.author, isFontLoaded && { fontFamily: 'Average' }]}>{activeQuote.author}</Text>

        <View style={styles.button}>
          <Button
            title={'Next quote'}
            onPress={this.nextQuote.bind(this)}
          />
        </View>
      </View>
    );
